...
// CHANGED FOR LAB 0
void asm_vmrun(struct Trapframe *tf, int num_runs) {
    ...
    //Dont this way deliberately to ensure scope encapsulation from calling function. no breaking changes.
	tf->tf_ds = num_runs;
	tf->tf_es = 0;
	unlock_kernel();
	asm(
...
int vmx_vmrun( struct Env *e ) {
    ...
    // NOTE: Original solution was:
    // if( curenv == NULL || e->env_runs == 1)
    // but due to "replace the conditional" from the professor I have elected to use this solution:
	if( e->env_runs == 1) {
